...
class UserData extends Model
{
    ...
    // day wise data two
    protected static function dateRangeWiseData($message_type,$customer_support, $temple_id, $date_from, $date_to)
    {
        ...
        $userList = $userList->whereRaw('(user_data.photo_url  is null or user_data.photo_url ="")');
    ...
    protected static function pendingApprovalData($customer_support, $temple_id)
    {
        ...
        $userList = $userList->select('user_data.name', 'user_data.user_mobile as mobile', 'user_data.created_at',
            'user_data.marital_status', 'user_data.last_seen', 'user_data.profile_percent', 'user_data.user_mobile as mobile', 'user_data.genderCode_user as gender', 'annual_income', 'user_data.id as user_id');
    ...
    {
        return UserData::with(['userpreference', 'userPhotos'])->where(['user_data.id' => $user_id])->first();
        // dd(UserData::with(['userpreference', 'userPhotos'])->where(['user_data.id' => $user_id])->first());
    }
    ...
    protected static function approvedProfiles($temple_id)
    {
        ...
        $userList = $userList->select('user_data.name', 'user_data.user_mobile as mobile', 'user_data.created_at',
        'leads.lead_value', 'leads.messege_send_count', 'user_data.marital_status as marital_status',
        'user_data.last_seen', 'user_data.profile_percent', 'user_data.user_mobile as user_mobile', 'leads.id as lead_id',
        'user_data.genderCode_user as gender', 'annual_income', 'user_data.id as user_id');
        $userList = $userList->orderBy('user_data.genderCode_user', 'DESC')->orderBy('user_data.annual_income', 'desc')->get();
        return $userList;
    }
    ...
    protected static function getFilteredData($min_age, $max_age, $min_height, $max_height, $castes, $marital_status, $manglik_status, $food_choice, $working_Status, $min_income, $filter_type, $send_profiles, $user_gender, $show_disabled,$religion)
    {
        $user_data = '';
       //dd($working_Status);
        $user_data = UserData::/*leftJoin('gender_mappings', 'gender_mappings.id', 'user_data.genderCode_user')
            ->leftJoin('occupation_mappings', 'occupation_mappings.id', 'user_data.occupationCode_user')
            ->leftJoin('educations', 'educations.id', 'user_data.educationCode_user')
            ->leftJoin('cities', 'cities.id',  'user_data.user_working_city')
            ->leftJoin('states', 'states.id',  'user_data.stateCode_user')
            ->leftJoin('countries', 'countries.id',  'user_data.countryCode_user')
            ->leftJoin('castes', 'castes.id',  'user_data.casteCode_user')
            ->leftJoin('manglik_mappings', 'manglik_mappings.id',  'user_data.manglikCode')
            ->where('user_data.maritalStatusCode', '!=',2)
            ->*/

        where(['user_data.is_deleted'=> 0, 'is_approved'=>1]);
        ...
        /*if(!empty($manglik_status) && $manglik_status!= 11){
            if($manglik_status==1 || $manglik_status == 4){
                $user_data = $user_data->whereRaw('manglikCode in(1, 4)');
            }else if($manglik_status == 2 || $manglik_status == 3){
                $user_data = $user_data->whereRaw('manglikCode in(2, 3)');
            } else if ($manglik_status == 2 || $manglik_status == 3) {
                $user_data = $user_data->where('manglikCode'. $manglik_status);
            }
        }*/
        ...
        // income filter
        if(!empty($min_income) && $min_income >0 && $filter_type > 0){
            $user_data = $user_data->whereBetween('monthly_income', [$min_income, $filter_type]);
        }
        ...
        // gender filter
        $user_data = $user_data->where('genderCode_user', '!=', $user_gender)->orderBy('user_data.created_at','desc')
        ->orderBy('user_data.is_premium', 'desc')/*->take(500)*/
        ->get(['user_data.name', 'user_data.monthly_income', 'gender', 'working_city as city','occupation','working_city' ,'manglik', 'caste', 'user_data.created_at', 'user_data.id as user_id', 'photo', 'is_premium', 'birth_date', 'height_int', 'weight', 'education','family_income','user_data.father_status','user_data.mother_status','user_data.marital_status']);
    ...
    {
        return UserData::whereRaw("id in ($profile_ids)")->orderBy('is_premium', 'desc')->orderBy('created_at','desc')->get(['name', 'birth_date', 'education', 'occupation', 'religion', 'caste', 'photo_url', 'height', 'weight', 'monthly_income', 'marital_status', 'citizenship', 'relation', 'about', 'working_city', 'city_family', 'birth_place', 'family_type', 'house_type', 'gotra', 'family_income', 'father_status', 'mother_status', 'unmarried_brothers', 'unmarried_sisters', 'married_brothers', 'married_sisters', 'photo', 'id as user_id']);
    }

    protected static function getTeleContactData($column_name)
    {
        return UserData::where($column_name, 0)->where(['is_delete'=>0, 'not_interested'=>0, 'is_paid'=>1])->whereRaw("name not like '%test%'")->orderBy('created_at','desc')->take(200)->get();
    }
    ...
    protected static function getSentTodayProfile($temple_id)
    {
        ...
        ->where('validity', '>=', "$today")->where(['user_data.temple_id'=>$temple_id, 'is_delete'=> 0, 'is_premium'=>1, 'profile_sent_day' => "$day" ])->get(['user_data.name', 'user_data.user_mobile', 'amount_collected_date', 'validity', 'user_data.id', 'user_data.birth_date']);
    }
    ...
    public static function isDisableCodeOrString($temple_id)
    {
        if($data == 0){
            return "no";
        }elseif ($data == 1) {
    ...
    public static function wishToGoAbroadCodeOrString($temple_id)
    {
        if($data == 0){
            return "no";
        }elseif ($data == 1) {
    ...
    public static function familyIncomeCodeOrString($temple_id)
    {
        if($data == 1){
            return "Hindu";
        }elseif ($data == 2) {
