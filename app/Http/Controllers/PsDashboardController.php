...
class PsDashboardController extends Controller
{
    ...
    public function getFilteredProfiles(Request $request)
    {
        ...
        if (!empty($array_list)) {
            $profile_lsit = implode(",", $array_list);
        } else {
            $profile_lsit = '';
        }
        ...
        $user_data = UserData::getFilteredData($request->min_age, $request->max_age, $request->min_height, $request->max_height, $request->castes, $request->marital_status, $request->manglik_status, $request->food_choice, $request->working, $request->min_income, $request->max_income, $profile_lsit, $user_gender->genderCode_user, $show_disabled, $request->religion);
        // dd($user_data);
        $dataset = array(
            "echo" => 1,
    ...
    public function getSampleFilteredProfiles(Request $request)
    {
        ...
        $user_data = UserData::getFilteredData($request->min_age, $request->max_age, $request->min_height, $request->max_height, $request->castes, $request->marital_status, $request->manglik_status, $request->food_choice, $request->working, $request->min_income, $request->max_income, $profile_lsit, $user_gender, $show_disabled, $request->religion);
    ...
    public function saveSendProfileList(Request $request)
    {
        ...
        $imploded_users = implode(',', $profile_list);

        $save_sent_match = '';
        $save_send_list = '';
        $save_send_list = Order::saveSendList($user_id, $imploded_users, Auth::user()->temple_id);

        for ($i = 0; $i < count($profile_list); $i++) {
            $save_sent_match = UserMatches::sendProfileList($user_id, $profile_list[$i]);
        }

        if ($save_sent_match  && $save_send_list) {
            DB::commit();
            return response()->json(['type' => true, 'message' => 'entry created', 'profile_list' => $imploded_users]);
        } else {
            DB::rollBack();
    ...
    public function CreateProfilePdfs(Request $request)
    {
        $user_data = '';
        $user_data = UserData::getProfilesInGroup($request->user_ids);
    ...
    public function weeklyProfileNotSentData(Request $request)
    {
        ...
        $order_list = DB::table("user_data")->join("userPreferences", "userPreferences.user_data_id", "user_data.id")
            ->whereRaw("userPreferences.validity >= '$today' and user_data.temple_id = '$temple_id' and user_data.is_premium = 1 ")
            ->select(["user_data.name", "user_data.gender", "validity", "birth_date", "user_data.id"])->get();
    ...
    public function historyDetails(Request $request)
    {
        ...
        $premium_meetings_list = PremiumMeetings::getAllPremiumMeetings($request->user_id,"3,4,5");
        ...
        if($request->generate_pdf){
            // dd($dataset);
            return view('personalized.profile-details-report',compact('user_detail_by_id','sent_profile_list','liked_by_me_profile_list','rejected_by_me_profile_list','liked_premium_meetings_list','rejected_premium_meetings_list','premium_meetings_list'));
        }else{
            return response()->json($dataset);
        }
    }
